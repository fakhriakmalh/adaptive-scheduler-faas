...
def train_models(history):
    ...
    lin_model.fit(X, y)
    lin_pred = lin_model.predict([[len(history)]])[0]  # Prediksi waktu berikutnya

    # Model Random Forest
    rf_model = RandomForestRegressor(n_estimators=10, random_state=42)
    rf_model.fit(X, y)
    rf_pred = rf_model.predict([[len(history)]])[0]  # Prediksi waktu berikutnya
...
def calculate_remaining_time(pid):
    ...
    lin_pred, rf_pred = train_models(history)
    ...
    tsi = (avg_burst_time + ewma_burst_time + lin_pred + rf_pred) / 4  # Rata-rata dari semua metode
