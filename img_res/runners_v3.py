...
ALPHA = 0.7  # Weight for remaining time
...
def calculate_wait_time(pid):
    """
    Calculate waiting time for a process
    \[
    \Wait Time = Current Time - Process Creation Time
    \]
    """
    if pid not in processTimestamps:
        return 0
...
def waitTermination(childPid):
    ...
    if processQueue:
        ...
        def priority_selector(process_item):
            ...
            wait_time = calculate_wait_time(pid)
            priority = (1 / remaining_time) + (BETA * wait_time) #Semakin kecil remaining_time, semakin besar prioritas
